    ...
    <div className="space-y-2">
      {monthDates.map((day) => {
        ...
        return <div key={day.date} className={`rounded-2xl border p-3 ${day.date === todayString() ? "border-blue-200 bg-blue-50" : "bg-white"}`}>
          ...
          {daySchedules.length === 0 ? <div className="mt-3 rounded-xl bg-neutral-50 px-3 py-2 text-sm text-neutral-400">休 / 未排班</div> : <div className="mt-3 grid gap-2">{daySchedules.map((schedule) => <div key={schedule.id} className="rounded-xl bg-neutral-900 px-3 py-2 text-sm text-white">
            <div className="font-bold">{schedule.department}｜{schedule.shiftName || "彈性班次"}</div>
            <div className="text-neutral-300">{schedule.startTime} - {schedule.endTime}</div>
          </div>)}</div>}
        </div>;
      })}
    </div>
...
function SchedulePanel({ setGlobalError }) {
  ...
  const [shiftForm, setShiftForm] = useState({ name: "烘焙早班", department: "烘焙坊", startTime: "08:00", endTime: "16:00", graceMinutes: DEFAULT_GRACE_MINUTES, isFlexible: false });
  ...
  async function createShiftTemplate(e) { e.preventDefault(); if (!shiftForm.name.trim()) return alert("請填寫班次名稱"); if (!shiftForm.isFlexible && minutesBetween(shiftForm.startTime, shiftForm.endTime) <= 0) return alert("下班時間必須晚於上班時間"); setSaving(true); const ok = await safeRun(() => addDoc(collection(db, "shiftTemplates"), { name: shiftForm.name.trim(), department: shiftForm.department, isFlexible: Boolean(shiftForm.isFlexible), startTime: shiftForm.isFlexible ? "" : shiftForm.startTime, endTime: shiftForm.isFlexible ? "" : shiftForm.endTime, scheduledMinutes: shiftForm.isFlexible ? 0 : minutesBetween(shiftForm.startTime, shiftForm.endTime), graceMinutes: Number(shiftForm.graceMinutes || DEFAULT_GRACE_MINUTES), createdAt: serverTimestamp(), updatedAt: serverTimestamp() }), "新增班次失敗。", setGlobalError); if (ok) await loadShiftTemplates(); setSaving(false); }
  ...
  async function assignShiftToCell(shift) { if (!quickCell || !quickCellEmployee || !shift) return; let customTimes = {}; if (shift.isFlexible) { const startTime = window.prompt("請輸入上班時間，例如 09:00", "09:00"); if (!startTime) return; const endTime = window.prompt("請輸入下班時間，例如 18:00", "18:00"); if (!endTime) return; customTimes = { startTime, endTime }; } setSaving(true); const ok = await writeSchedule(quickCellEmployee, shift, quickCell.date, "", customTimes); if (ok) { setQuickCell(null); await loadSchedules(); } setSaving(false); }
